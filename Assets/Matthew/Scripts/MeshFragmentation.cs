...
namespace GK
{
	public class MeshFragmentation : MonoBehaviour
	{
		...
		public float MinImpactToBreak = 0.0f;
		...
		{
			Reload();
			if (Rigidbody.mass < 200) // We'll destroy any fragments less than a certain mass for performance reasons
			...
			if (area > MinBreakArea)
			{
				...
				for (int i = 0; i < sites.Length; i++)
				{
					...
					if (clipped.Count > 0)
					{
						...
						var bs = newGo.GetComponent<MeshFragmentation>();
