...
public class MeshDestroy : MonoBehaviour
{
    ...
    void Start()
    {
        ...
        if (rbody.mass < 10) // We'll destroy any fragments less than a certain mass for performance reasons
    ...
    {
        if (other.gameObject.name == "Explosive Plasma Bolt(Clone)" || other.gameObject.name == "Explosion(Clone)" || other.gameObject.name == "Melee(Clone)")
        {
        ...
        if (other.gameObject.name == "Bullet(Clone)" && isGlass)
        {
            if (breakingSound != null)
                AudioSource.PlayClipAtPoint(breakingSound, transform.position);
            DestroyMesh();
    ...
    private void DestroyMesh()
    {
        ...
        for (var i = 0; i < parts.Count; i++)
        {
            parts[i].MakeGameobject(this, parts.Count, rbody);
    ...
    public class PartMesh
    {
        ...
        public void MakeGameobject(MeshDestroy original, int fragments, Rigidbody rbody)
        {
            ...
            rigidbody.mass = rbody.mass / fragments;
