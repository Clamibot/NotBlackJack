...
public class DestructibleTerrain : MonoBehaviour
{
    ...
    {
        if (other.gameObject.name == "Explosive Plasma Bolt(Clone)" || other.gameObject.name == "Explosion(Clone)" || other.gameObject.name == "Melee")
        {
    ...
    {
        if (other.gameObject.name == "Grenade(Clone)" && isGlass)
        {
            if (breakingSound != null)
                AudioSource.PlayClipAtPoint(breakingSound, transform.position);
            Explode(transform);
    ...
    void Explode(Transform target)
    {
        ...
        int trianglesToUse = triangles.Length;

        if (trianglesToUse > 60)
            trianglesToUse = 60; // Limit the number of triangles in the explosion for performance reasons

        // get each face
        for (int i = 0; i < trianglesToUse; i += 3)
        {
